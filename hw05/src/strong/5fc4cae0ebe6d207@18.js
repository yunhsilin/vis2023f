...
function _strong(d3,output,drag,url_list,invalidation)
{
  // 指定圖表的尺寸。
  const width = 2000;
  const height = 400;
  ...
  // 計算圖形並啟動力模擬。
  const root = d3.hierarchy(output.children[13]);
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("strong")).define("strong", ["d3","output","drag","url_list","invalidation"], _strong);
  return main;
}
